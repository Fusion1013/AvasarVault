...
# Organization
```dataview
TABLE description AS "Description" FROM #organization/major WHERE contains(nation, "So'Dar Empire")
```
# Economy
...
# Timeline
```dataview
TABLE WITHOUT ID "[[" + file.name + "]]" + choice(contains(file.etags, "#city_founding"), " `far:Flag`", "") + choice(contains(file.etags, "#discovery"), " `far:Lightbulb`", "") + choice(contains(file.etags, "#nation_founding"), " `ris:Flag`", "") + choice(contains(file.etags, "#law"), " `ris:Book2`", "") AS "Event", description AS "Description" FROM #timeline_event WHERE contains(nation, "So'Dar Empire") SORT date DESC
```
